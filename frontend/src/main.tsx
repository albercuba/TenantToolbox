...
function App() {
  ...
  return (
    <>
      ...
      <main className="main">
        {activeView === "dashboard" ? <Dashboard apiStatus={apiStatus} onTenants={() => navigate("tenants")} activeTenant={managedTenants.find((tenant) => tenant.domain === selectedTenantId)?.name} /> : <TenantList token={token} />}
      </main>
    </>
  );
}
