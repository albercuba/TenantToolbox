...
The frontend must use the shared `../Templates/PharmaPMS/ui-template` as its visual reference. Preserve its Desk-style layout, Inter typography, compact spacing, card/table patterns, and status color semantics while replacing pharmacy-specific content with TenantToolbox concepts. Copy only the reusable style system into this repository; do not make TenantToolbox depend on the template project at runtime. The sidebar uses the reference button reset, full-width spacing, active-state tint, and responsive drawer behavior.
